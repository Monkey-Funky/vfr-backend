...
public sealed class GetSizeRecommendationQueryHandlerTests
{
    ...
    public async Task Handle_NoAvatar_ThrowsNotFoundException()
    {
        ...
        // When no avatar exists for the customer, the handler throws NotFoundException
        // (which maps to HTTP 404). This is consistent with:
        //   - The handler implementation: throw new NotFoundException("Avatar", customerId)
        //   - The integration test: GetSizeRecommendation_WithoutAvatar_ShouldReturn404
        // BusinessRuleException (422) is reserved for rule violations on an existing resource,
        // not for a missing entity.
