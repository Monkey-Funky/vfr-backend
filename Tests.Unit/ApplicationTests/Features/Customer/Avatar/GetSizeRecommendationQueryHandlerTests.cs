...
//        // FIX U-10: WithMessage("*NO_AVATAR*") tests Exception.Message, which contains the
//        // human-readable string — not the Code. Use .Where(ex => ex.Code == ...) to assert
//        // directly on the BusinessRuleException.Code property that the test intends to check.
//        await act.Should().ThrowAsync<BusinessRuleException>()
//            .Where(ex => ex.Code == "NO_AVATAR");
//    }
