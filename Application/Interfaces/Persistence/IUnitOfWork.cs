...
public interface IUnitOfWork : IAsyncDisposable
{
    ...
    /// Use when multiple SaveChangesAsync calls (including those from domain event handlers)
    /// must commit atomically or roll back together.
    ///
    /// The returned IDbContextTransaction must be disposed; use <c>await using</c>.
    /// Commit explicitly with CommitAsync; on any exception, rollback with RollbackAsync.
    /// </summary>
