...
/// including the full StockAdjustments audit trail (ordered by AdjustedAt DESC).
/// Throws NotFoundException (→ 404) when the record does not exist or does not
/// belong to the authenticated retailer.
/// RetailerId is resolved from the JWT — IDOR protection.
/// </summary>
public sealed record GetInventoryByProductIdQuery(Guid ProductId)
    : IRequest<InventoryDetailDto>;