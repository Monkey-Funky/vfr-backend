...
public sealed class AuthController : PublicController
{
    ...
    [SwaggerOperation(
        "Google OAuth login",
        "Validates a Google ID token. Creates a new retailer account on first sign-in. " +
    ...
    {
        // FIX F-01: LogoutCommand has no request body — construct inline.
...
{
    /// <summary>The step token returned by RegisterStep1 (send as a form field, NOT Authorization header).</summary>
    public string TempStepToken { get; set; } = string.Empty;

    /// <summary>Business category (e.g. "Fashion", "Electronics").</summary>
